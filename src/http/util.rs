...
pub fn internal_server_error<T: std::fmt::Debug>(
    ...
    let mut not_found = Response::new(Body::from(html::render_page(
...
pub fn not_found_response() -> Result<Response<Body>, hyper::Error> {
    let mut not_found = Response::new(Body::from(html::render_page(html::common::not_found())));
    *not_found.status_mut() = StatusCode::NOT_FOUND;
    Ok(not_found)
}

pub fn bad_request_response<T: Into<String>>(message: T) -> Result<Response<Body>, hyper::Error> {
    let mut not_found = Response::new(Body::from(html::render_page(html::common::bad_request(
        message,
    ))));
